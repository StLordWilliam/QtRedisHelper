...
Reader::Reader(QString host, int port)
{
	...
	connect(socket, SIGNAL(connected()), this, SLOT(socketConnected())/*, Qt::QueuedConnection*/);
	connect(socket, SIGNAL(disconnected()), this, SLOT(socketConnectionClosed())/*, Qt::QueuedConnection*/);
	connect(socket, SIGNAL(readyRead()), this, SLOT(socketReadyRead())/*, Qt::QueuedConnection*/);
...
void Reader::readLine(const QString &reply)
{
    ...
    } else if(first=='*')
    {
		...
		QStringList lstReturn = reply.split("\r\n");
		for (int index = 1; index < lstReturn.size();index++)
		{
			QString strReturn = lstReturn[index];
			strReturn.remove(":");
			if (strReturn != "")
			{
				result.push_back(strReturn);
			}
		}
    }
