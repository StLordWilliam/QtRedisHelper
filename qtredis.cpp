		...
		, reader(nullptr)
{
	...
	bool bRet = connect(reader, SIGNAL(response(QVariant)), this, SLOT(response(QVariant)),Qt::DirectConnection);
}
...
bool QtRedis::openConnection()
{
    ...
    QEventLoop loop;
	bool bRet = connect(reader, SIGNAL(connected()), &loop, SLOT(quit()), Qt::DirectConnection);
    loop.exec();
...
void QtRedis::connectHost(const QString &host, const quint16 port)
{
	...
	bool bRet = connect(reader, SIGNAL(connected()), this, SLOT(slotConnected()));
}
...
{
	bool bRet = connect(reader, SIGNAL(disconnected()), this, SLOT(slotDisconnected()));
    emit connected();
}
...
QtRedis::Reply QtRedis::command(QString cmd)
{
	...
	{		
		...
		if ((bGetResponse) && (!responseData.isEmpty()) && (responseData.size() > 1))
		{
			QString strResponseData0 = responseData[0];
			if (strResponseData0 == "integer")
			{
				reply.type = strResponseData0;
				reply.value = responseData[1].toInt();
			}
			else if (strResponseData0 == "list")
			{
				reply.type = strResponseData0;

				QStringList list;
				//for (int i = 1; i < responseData.length(); i++)
				//int nSize = responseData.size();
...
//add by liuwei 2021-06-30		新增bloom fliter 指令
//检测是否已存在
bool QtRedis::bf_exists(QString key, QString bfname)
{
	QString cmd("bf.exists ");
	cmd.append(bfname);
	cmd.append(" ");
	cmd.append(key);
...
//添加至bf中
bool QtRedis::bf_add(QString key, QString bfname)
{
	QString cmd("bf.add ");
	cmd.append(bfname);
	cmd.append(" ");
...
//批量检测是否已存在
bool QtRedis::bf_mexists(QStringList lstkey, QString bfname, QMap<QString, bool> &mapKey2IsExist)
{
	QString cmd("bf.mexists ");
	cmd.append(bfname);

	for (int i = 0; i < lstkey.size(); i++)
	{
		QString key = lstkey[i];
		cmd.append(" ");
		cmd.append(key);
	}
	...
	QStringList lstValue = reply.value.toStringList();
	if (lstValue.size() == 0)
	{
		qWarning() << "bf_mexists failed! lstValue.size() == 0 ";
		return false;
	}

	int nSize = lstkey.size() > lstValue.size() ? lstValue.size() : lstkey.size();
	
	for (int i = 0; i < nSize; i++)
	{
		QString strValue = lstValue[i];
		QString strKey = lstkey[i];
		bool bIsExist = strValue == "1" ? true : false;
		mapKey2IsExist.insert(strKey, bIsExist);
	}
...
//批量添加至bf中
bool QtRedis::bf_madd(QStringList lstkey, QString bfname, QMap<QString, bool> &mapKey2IsSuccess)
{
	QString cmd("bf.madd ");
	cmd.append(bfname);
	...
	if (lstValue.size() == 0)
	{
		qWarning() << "bf_madd failed! lstValue.size() == 0 ";
		return false;
	}
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		QString strKey = lstkey[i];
		bool bIsExist = strValue == "1" ? true : false;
		mapKey2IsSuccess.insert(strKey, bIsExist);
	}
