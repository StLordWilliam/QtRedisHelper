...
public:
	...
	//检测是否已存在
	bool bf_exists(QString key,QString bfname);
	//添加至bf中
	bool bf_add(QString key, QString bfname);

	//批量检测是否已存在
	bool bf_mexists(QStringList lstkey, QString bfname,QMap<QString, bool> &mapKey2IsExist);
	//批量添加至bf中
	bool bf_madd(QStringList lstkey, QString bfname, QMap<QString, bool> &mapKey2IsSuccess);
